...
public class PaymentController {
    ...
    public ResponseEntity<PaymentResponse> initiatePayment(@Valid @RequestBody PaymentRequest paymentRequest, @AuthenticationPrincipal UserPrincipal currentUser) {
        PaymentResponse paymentResponse = paymentService.initiatePayment(paymentRequest, currentUser.getUsername());
        return ResponseEntity.ok(paymentResponse);
    }

    @GetMapping("/{paymentId}")
    public ResponseEntity<PaymentResponse> getPaymentDetails(@PathVariable Long paymentId, @AuthenticationPrincipal UserPrincipal currentUser) {
